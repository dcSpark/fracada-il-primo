...
import qualified PlutusTx               (FromData (fromBuiltinData),
                                         ToData (toBuiltinData), applyCode,
                                         compile, liftCode, makeIsDataIndexed,
                                         makeLift)
import           PlutusTx.Builtins      as Builtins
import           PlutusTx.Prelude       as P
import           Prelude                (Show, String)

newtype AddToken = AddToken [Signature]
...
instance Scripts.ValidatorTypes Fractioning where
  type RedeemerType Fractioning = Maybe AddToken
  type DatumType Fractioning = FractionNFTDatum
...
{-# INLINEABLE fractionNftValidator #-}
fractionNftValidator :: FractionNFTParameters -> FractionNFTDatum -> Maybe AddToken -> ScriptContext -> Bool
fractionNftValidator FractionNFTParameters {authorizedPubKeys, minSigRequired} FractionNFTDatum {tokensClass, totalFractions} redeemer ctx =
  let txInfo = scriptContextTxInfo ctx
   ...
   in if isJust redeemer
        then -- adding nfts or minting, signatures are required

          let AddToken signatures' = fromJust' redeemer
           ...
           in if forgedTokens > 0
                then -- minting more fractional tokens path

                  let -- keep the NFTs ( the value locked doesn't change)
                  ...
                  if forgedTokens < 0
                    then -- we don't allow partial burn of tokens
                      False
                    else -- add new tokens to the lock

                      let --extract the value increase
                       ...
                       in traceIfFalse "not enough signatures to add tokens" requiredSignatures
        ...
        else -- return all the NFTs

          let -- make sure no nfts are left in the contract
           ...
           in traceIfFalse "not all NFTs returned" allAssetAreReturned
...
fractionNftValidatorInstance params =
  Scripts.mkTypedValidator @Fractioning
